...
class TriggerSystem:
    ...
    def evaluate(
        ...
        # 2e. Штраф за чужие имена — если в начале сообщения есть имя,
        # но это НЕ имя бота, это скорее всего обращение к другому человеку
        first_word = text_lower.split()[0].rstrip(".,!?:") if text_lower.strip() else ""
        if first_word and first_word not in all_names:
            # Проверяем что это похоже на имя (короткое слово, не частица/предлог)
            common_names = {"андрей", "алекс", "алёна", "алена", "мария", "маша", "дима", "дим", "сергей", "серега",
                           "олег", "igor", "игорь", "наташ", "натали", "лен", "лена", "елен", "макс", "максим",
                           "ваня", "иван", "петя", "петр", "коля", "никол", "саш", "сань", "женя", "евген"}
            if first_word in common_names or (len(first_word) >= 3 and first_word.isalpha() and first_word[0].isupper() or text_lower[0].isupper()):
                score -= 0.3
                reasons.append(f"not_bot_name:{first_word}")
