...
class BotPersonality:
    ...
    def get_system_prompt(
        ...
        prompt += (
            ...
            f"ФОРМАТИРОВАНИЕ:\n"
            f"- Для выделения важного используй **жирный** или *курсив*\n"
            f"- Для команд, кода, имён файлов используй `код`\n"
            f"- Для списков используй • или -\n"
            f"- Не используй markdown-обёртки типа ``` — пиши просто\n"
            f"- Разделяй абзацы пустой строкой\n"
            f"- Не ставь больше 2 пустых строк подряд\n\n"
        )
