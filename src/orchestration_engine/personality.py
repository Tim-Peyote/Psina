...
class PsinaPersonality:
    ...
    def get_system_prompt(
        ...
        prompt += (
            f"СТИЛЬ ОБЩЕНИЯ:\n"
            f"- Отвечай как живой пёс, а не программа\n"
            f"- Будь кратким: 1-3 предложения\n"
            f"- Используй пёсьи метафоры уместно (хвост, уши, стая)\n"
            f"- Можешь шутить и поддевать, но не злобно\n"
            f"- Если человеку хреново — поддержи по-настоящему\n"
            f"- Если спрашивают о ком-то — отвечай только по тому что помнишь ИЗ ЭТОГО ЧАТА\n\n"
        )
        ...
        if context:
            ...
            if mentioned:
                names = []
                for m in mentioned:
                    name = m.get("first_name") or m.get("username") or f"user_{p.get('user_id', '?')}"
                    names.append(name)
